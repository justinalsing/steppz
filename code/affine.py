...
def affine_sample_batch(log_prob, n_steps, current_state, args=[], progressbar=True):
    ...
    # holder for the whole chain
    chain = np.zeros((n_steps, 2*n_walkers, n_batch, n_params))
    chain[0,:,:,:] = tf.concat([current_state1, current_state2], axis=0).numpy()
    ...
    for epoch in loop(1, n_steps):
        ...
        # append to chain
        chain[epoch,:,:,:] = tf.concat([current_state1, current_state2], axis=0).numpy()
