...
@tf.function
def log_likelihood_normal(fluxes, flux_variances, predicted_fluxes, predicted_flux_variances, n_sigma_flux_cuts):
    ...
    # selection cuts (CDF of the Student's-t for each band given the flux limits)
    log_selection_ = tf.reduce_sum(-tf.math.log(tf.multiply(0.5, tf.subtract(1., tf.math.erf(tf.divide(tf.subtract(flux_cuts, predicted_fluxes), tf.multiply(tf.math.sqrt(2.), predicted_flux_sigmas) ))))), axis=-1)
