...
training_theta = np.concatenate([np.load('/cfs/home/alju5794/steppz/sps_models/model_HMII/training_data/parameters/parameters{}.npy'.format(i)) for i in range(32)], axis=0)
training_mags = np.concatenate([np.load('/cfs/home/alju5794/steppz/sps_models/model_HMII/training_data/photometry/KV_photometry{}.npy'.format(i)).astype(np.float32) for i in range(32)], axis=0) # units: nanomaggies

# transform parameters
training_theta[:,-3] = np.exp(training_theta[:,-3]) # log(tmax) -> tmax
