...
if burnin is True:
    ...
    initial_latent_chain = np.load('/cfs/home/alju5794/steppz/kids/chains/B_BHM_GAMA_validation-run_with_selection/latent0.npy').astype(np.float32)
    ...
    initial_nuisance_chain = np.load('/cfs/home/alju5794/steppz/kids/chains/B_BHM_GAMA_validation-run_with_selection/nuisance0.npy').astype(np.float32)
    nuisance_current_state = [tf.convert_to_tensor(initial_nuisance_chain[0:n_nuisance_walkers,...], dtype=tf.float32), tf.convert_to_tensor(initial_nuisance_chain[n_nuisance_walkers:2*n_nuisance_walkers,...], dtype=tf.float32)] 
    nuisance_parameters_ = nuisance_current_state[np.random.randint(0, 2)][np.random.randint(0, n_nuisance_walkers),...]

    initial_hyper_chain = np.load('/cfs/home/alju5794/steppz/kids/chains/B_BHM_GAMA_validation-run_with_selection/hyper0.npy').astype(np.float32)
    ...
    hyper_parameters_ = hyper_current_state[np.random.randint(0, 2)][np.random.randint(0, n_hyper_walkers),...]
