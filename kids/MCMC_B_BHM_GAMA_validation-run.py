...
# import data
fluxes, flux_sigmas, zspec, zprior_sig, parameters = pickle.load(open('/cfs/home/alju5794/steppz/kids/mock_data/KVxGAMA-like_mock-selected.pkl', 'rb'))
...
zprior_sig_fixed = tf.ones(zprior_sig.shape, dtype=tf.float32)*1e-2
...
def log_latentparameter_conditional(latentparameters, hyperparameters, fluxes, flux_variances, zspec, zprior_sig, nz_parameters, nz_fiducial):
    ...
    # parameters
    logits, locs, logscales, skewness, tailweight = tf.split(nz_parameters, (2, 2, 2, 2, 2), axis=-1)
...
def log_nz_conditional(theta, z):
    ...
    # pull out parameters
    logits, locs, logscales, skewness, tailweight = tf.split(theta, (2, 2, 2, 2, 2), axis=-1)
...
# initialize n(z)
logits = np.array([0.67322165, 1.3267788]).astype(np.float32)
locs = np.array([0.1756928, 0.653366]).astype(np.float32)
scales = np.array([0.10497721, 0.19987684]).astype(np.float32)
skewness = np.array([0.5169175 , 0.23451573]).astype(np.float32)
tailweight = np.array([0.346626 , 1.3105514]).astype(np.float32)
nz_parameters_ = tf.concat([logits, locs, tf.math.log(scales), skewness, tailweight], axis=-1)
nz_current_state = [nz_parameters_ + tf.random.normal([n_nz_walkers, nz_parameters_.shape[0]], 0, 1e-2), nz_parameters_ + tf.random.normal([n_nz_walkers, nz_parameters_.shape[0]], 0, 1e-2)]

# fiducial nz
logits, locs, logscales, skewness, tailweight = tf.split(nz_parameters_, (2, 2, 2, 2, 2), axis=-1)
...
# save the chain
np.save('/cfs/home/alju5794/steppz/kids/chains/B_BHM_GAMA_validation-run/latent{}.npy'.format(1), sps_prior.bijector(latent_samples_).numpy().astype(np.float32)[np.random.randint(0, 2*n_latent_walkers, n_thin),...] )
np.save('/cfs/home/alju5794/steppz/kids/chains/B_BHM_GAMA_validation-run/z{}.npy'.format(1), sps_prior.bijector(latent_samples_).numpy().astype(np.float32)[np.random.randint(0, 2*n_latent_walkers, n_thin),:,-1] )
...
#    nz_parameters_ = nz_current_state[np.random.randint(0, 2)][np.random.randint(0, n_nz_walkers),...] 
    ...
    # save the chain
    np.save('/cfs/home/alju5794/steppz/kids/chains/B_BHM_GAMA_validation-run/latent{}.npy'.format(step+2), sps_prior.bijector(latent_samples_).numpy().astype(np.float32)[np.random.randint(0, 2*n_latent_walkers, n_thin),...] )
    np.save('/cfs/home/alju5794/steppz/kids/chains/B_BHM_GAMA_validation-run/z{}.npy'.format(step+2), sps_prior.bijector(latent_samples_).numpy().astype(np.float32)[np.random.randint(0, 2*n_latent_walkers, n_thin),:,-1] )
