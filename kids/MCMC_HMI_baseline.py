...
root_dir = '/cfs/home/alju5794/steppz/sps_models/model_HMI/trained_models/'
...
sps_prior = ModelHMIBaselinePrior()
...
n_walkers = 300
latent_current_state = [tf.convert_to_tensor(np.load('/cfs/home/alju5794/steppz/kids/initializations/HMI_walkers_phi.npy')[0:n_walkers,:,:].astype(np.float32), dtype=tf.float32), tf.convert_to_tensor(np.load('/cfs/home/alju5794/steppz/kids/initializations/HMI_walkers_phi.npy')[n_walkers:2*n_walkers,:,:].astype(np.float32), dtype=tf.float32)]
...
#    print('batch {} took {}min'.format(batch, (time.time() - start)/60.))
    ...
    # save the chain
    np.save('/cfs/home/alju5794/steppz/kids/chains/HMI_baseline_batch{}.npy'.format(batch), sps_prior.bijector(latent_samples[-keep:,:,:,:].astype(np.float32)).numpy() )
